"""
1. Give the characteristics of the opencl compatible devices installed on your PC:
- Device reference.
- Global, local, cache Memories sizes.
- Number of compute units (Streaming MultiProcessors).
- Max number of work items, work groups, etc....
"""
...
for platform in pyopencl.get_platforms():
    for device in platform.get_devices():
        ...
        print(
            "\tGlobal cache: \t\t"
            + str(device.global_mem_cache_size / 2**10)
            + " KB ("
            + pyopencl.device_mem_cache_type.to_string(device.global_mem_cache_type)
            + ")"
        )
        print("\tGlobal cache line: \t" + str(device.global_mem_cacheline_size) + " B")
        print(
            "\tLocal memory: \t\t"
            + str(device.local_mem_size / 2**10)
            + " KB ("
            + pyopencl.device_local_mem_type.to_string(device.local_mem_type)
            + ")"
        )
        print(
            "\tConstant memory: \t"
            + str(device.max_constant_buffer_size / 2**10)
            + " KB"
        )

...
__kernel void mmul(const int M, const int N, const int K,
    ...
    // Thread identifiers
    int globalRow = offsetM + tidm + WPTM*RTSM;
    int globalCol = offsetN + tidn + WPTM*RTSN;
...
return;
    ...
    for (int wm=0; wm<WPTM; wm++) {
        globalRow = offsetM + tidm + wm*RTSM;
        for (int wn=0; wn<WPTN; wn++) {
            globalCol = offsetN + tidn + wn*RTSN;
            C[globalCol*M + globalRow - GPU_BUFFER_SIZE] = acc[wm][wn];
        }
    }
}
